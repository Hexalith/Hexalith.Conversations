...
1. **Folder+id dispatch** (`{spec_folder}` and `{story_id}` are set): the write-back always lands at the id-keyed story spec. The `/home/administrator/projects/hexalith/conversations/_bmad-output/implementation-artifacts` fallback in step 2 below is never used in this mode, even for halts before planning starts.
...
2. **Otherwise:**
   ...
   - If `{spec_file}` is unknown or missing, create `/home/administrator/projects/hexalith/conversations/_bmad-output/implementation-artifacts/bmad-dev-auto-result-<slug-or-timestamp>.md` with:
     ```markdown
     ---
     status: <final status>
     ---
