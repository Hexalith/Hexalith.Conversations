...
3. Read `./spec-template.md` fully. Fill it out based on the intent and investigation, resolving the template's `date` field to the current system date. Populate `submodule_promotions` with `[]` for non-promotion work or with every affected root-declared `references/...` path exactly once as `{ path, require_remote }`. Set `require_remote` to `true` for every path whose submodule commit must already exist in a locally known remote-tracking ref before completion — that is the default for any `references/...` submodule shared with other clones, because an unpushed commit leaves the umbrella recording a gitlink no other clone can resolve. Use `false` only for a submodule deliberately kept local to this workspace, and say why in the plan. A missing declaration in existing work may be filled automatically only as an exact transcription of already-approved scope; ambiguous or expanded scope requires Product Owner or user approval and must be treated as an intent gap. If `preserved_intent` is non-empty, replace the `<frozen-after-approval>` block in the spec you just filled out with `preserved_intent`, before writing. Write the result to `{spec_file}`.
...
6. Token count check (see SCOPE STANDARD). If spec exceeds 1600 tokens:
   ...
   - On **S**: Propose the split — name each secondary goal. For each deferred goal, append one new entry to `/home/administrator/projects/hexalith/conversations/_bmad-output/implementation-artifacts/deferred-work.md` using this format. Do not modify existing entries or look for duplicates. Rewrite the current spec to cover only the main goal — do not surgically carve sections out; regenerate the spec for the narrowed scope. Continue to checkpoint.
     ```markdown
