// <copyright file="RedactMessageContentValidation.cs" company="ITANEO">
...
/// <summary>
/// Validates redaction domain commands without external lookups.
/// </summary>
internal static class RedactMessageContentValidation
{
    public static ConversationRejectedDomainEvent? Validate(RedactMessageContent? command, ConversationState? state)
    {
        ...
        RedactMessageContentCommand publicCommand = command.PublicCommand;
        ...
        ConversationState? state)
    {
        ...
        if (state.LastEventAt is { } lastEventAt && publicCommand.OperationTimestamp < lastEventAt)
        {
            return Reject(
                ConversationErrorCode.CommandValidationFailed,
                "redaction_timestamp_not_monotonic",
                publicCommand.Metadata.SchemaVersion,
                publicCommand.Metadata.CorrelationId,
                publicCommand.Metadata.CausationId);
        }

        if (state.TryGetRedaction(ConversationState.RedactionTargetKey(publicCommand.Target), out ConversationRedactionState? redaction)
            && redaction is not null
            && !IsCompatible(publicCommand, redaction))
        {
            return Reject(
    ...
    public static bool IsCompatibleExistingRedaction(RedactMessageContentCommand command, ConversationState? state)
    {
        ...
        return state is not null
            && state.TryGetRedaction(ConversationState.RedactionTargetKey(command.Target), out ConversationRedactionState? redaction)
            && redaction is not null
            && IsCompatible(command, redaction);
    }
    ...
    public static bool IsCompatible(RedactMessageContentCommand command, ConversationRedactionState redaction)
    {
        ArgumentNullException.ThrowIfNull(command);
        ArgumentNullException.ThrowIfNull(redaction);

        return redaction.Target == command.Target
            && redaction.Category == command.Category
            && redaction.PolicyReference == command.PolicyReference
            && redaction.Rationale == command.Rationale;
    }
