// <copyright file="RedactMessageContentCommandHandler.cs" company="ITANEO">
...
/// <summary>
/// Handles governed redaction commands behind tenant, governance, audit, and idempotency gates.
/// </summary>
public sealed class RedactMessageContentCommandHandler
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="RedactMessageContentCommandHandler"/> class.
    /// </summary>
    ...
    public RedactMessageContentCommandHandler(
    ...
    /// <summary>
    /// Executes the redaction command after fail-closed authorization and audit checks.
    /// </summary>
    /// <param name="command">The public redaction command.</param>
    ...
    /// <returns>A redaction event, idempotency replay, no-op, or typed rejection.</returns>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ConversationRejectedDomainEvent? schemaRejection = RedactMessageContentBoundary.ValidateSchemaShape(command);
        ...
        return await ConversationTenantAccessGuard.RunAsync(
            ...
            async guardedCancellationToken =>
            {
                ConversationRejectedDomainEvent? semanticRejection =
                    RedactMessageContentBoundary.ValidateSemanticShape(command!, eventId);
                ...
                {
                    return await ExecuteRedactionMutationAsync(
                        command!,
                        trustedTenantId,
                        loadStateAsync,
                        eventId,
                ...
                return await _idempotencyExecutor.ExecuteAsync(
                    ...
                    eventId,
                    causationId: null,
                    mutationCancellationToken => ExecuteRedactionMutationAsync(
                        command,
                        trustedTenantId,
                        loadStateAsync,
                        eventId,
    ...
    private async ValueTask<DomainResult> ExecuteRedactionMutationAsync(
        RedactMessageContentCommand command,
        ...
        CancellationToken cancellationToken)
    {
        ...
        ConversationRejectedDomainEvent? preAuditRejection =
            RedactMessageContentBoundary.ValidateStateBeforeAudit(command, eventId, state);
        if (preAuditRejection is not null)
        {
            return DomainResult.Rejection(new IRejectionEvent[] { preAuditRejection });
        }

        if (RedactMessageContentBoundary.IsCompatibleExistingRedaction(command, state))
        {
        ...
        ConversationRejectedDomainEvent? auditPairingRejection =
            RedactMessageContentBoundary.ValidateAuditEvidenceProvided(command, auditResult.Evidence);
        if (auditPairingRejection is not null)
        {
            return DomainResult.Rejection(new IRejectionEvent[] { auditPairingRejection });
        }

        return RedactMessageContentBoundary.DispatchValidated(command, auditResult.Evidence, eventId, state);
    }
