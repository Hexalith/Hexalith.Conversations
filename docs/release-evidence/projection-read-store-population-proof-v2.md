...
The consequence is that the CREATE and APPEND rows carry no usable signal at a +-5% threshold: their round-to-round spread is several times the gate itself, so neither a `pass` nor a `fail` on those two rows may be cited as evidence about a regression. **LIST and OPEN are a different matter.** At +329.9% and +1760.9% they are orders of magnitude outside any variance argument, they reproduced across both measurement rounds, and they are real. The frozen 5% envelope is unchanged and no amendment is sought; warm-up discard and a more robust statistic belong to whatever run re-measures after those two regressions are actually fixed. **SM-C2 remains an open release blocker** while any row exceeds the frozen 5% threshold, so this proof's overall result is fail even though its functional boundary lanes pass.
