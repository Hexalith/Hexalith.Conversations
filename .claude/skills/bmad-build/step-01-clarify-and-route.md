...
1. Load context.
   ...
   - **Determine context strategy.** Using the intent and the artifact listing, infer whether the current work is a story from an epic. Do not rely on filename patterns or regex — reason about the intent, the listing, and any epics file content together.
     ...
     **A) Epic story path** — if the intent is clearly an epic story:
     ...
     4. **Verify.** After compilation, repeat the complete frontmatter, authority-identity, heading, and freshness validation from step 2. If valid, load it. If verification fails or the governing historical authority cannot be inspected, HALT and report the stable failure; do not accept heading-only context.
