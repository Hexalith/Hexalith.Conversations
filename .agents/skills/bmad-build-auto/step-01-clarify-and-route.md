...
1. Load context.
   ...
   - **Determine context strategy.** Using the intent and the artifact listing, infer whether the current work is a story from an epic. Do not rely on filename patterns or regex — reason about the intent, the listing, and any epics file content together.
     ...
     **A) Epic story path** — if the intent is clearly an epic story:
     ...
     2. **Check for a valid cached epic context.** Look for `{{.implementation_artifacts}}/epic-<N>-context.md` (where `<N>` is the epic number). A file is **valid** only when it exists, is non-empty, begins with closed YAML frontmatter containing exactly one non-empty `overlay_version` and exactly one non-empty `architecture_version`, the next content heading is `# Epic <N> Context:` with the correct epic number, both identities exactly match the planning authority that governs the target epic, and the exact governing authority slice has not changed. Do not use filesystem mtime alone: a later append-only overlay that explicitly preserves the target's historical authority does not make that historical context stale. For Epic 6 under the V8 identity pair, validity additionally requires exactly one `### 6.1 ` through `### 6.12 ` heading and the detailed V8 preservation, landing-zone, dependency, evidence, gate, and hold semantics required by `[[bmad-snapshot:compile-epic-context.md]]`. Missing, duplicate, malformed, unresolvable, incomplete, or mismatched identity evidence makes the cache invalid; never substitute the current checkout or a newer unrelated authority identity.
     ...
     4. **Verify if compiled.** If epic context was compiled, repeat the complete frontmatter, authority-identity, heading, and freshness validation from step 2. If valid, load it. If verification fails or governing historical authority is unavailable, HALT with status `blocked` and blocking condition `context compilation evidence invalid`; never accept heading-only context.
