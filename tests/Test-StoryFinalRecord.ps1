...
function Test-PathSets {
    ...
    $expectedSet = @($Expected | ForEach-Object { ConvertTo-NormalizedPath $_ } | Sort-Object -Unique)
    $observedSet = @($Observed | ForEach-Object { ConvertTo-NormalizedPath $_ } | Sort-Object -Unique)
...
function Test-TrxResult {
    ...
    $contractResult = $trx.SelectNodes("//*[local-name()='UnitTestResult']") |
...
function Test-LiveRecord {
    ...
    $recordAbsolutePath = Join-Path $Root $recordPath
    ...
    $trx = Test-TrxResult -Path $ResolvedTestResultPath -ExpectedCounts $live.expectedCounts -ContractTestName ([string]$live.contractTestName) -Failures $failures
    ...
    return [ordered]@{
        ...
        publicContractShape = [ordered]@{
            ...
            baselineSha256 = Get-Sha256ForFile (Join-Path $Root $contractBaselinePath)
...
function Test-HistoricalRecord {
    ...
    if (@($Record.fileListAmendments).Count -gt 0) {
        ...
        $amendmentAbsolutePath = Join-Path $Root $amendmentRecordPath
    ...
    $rawDiff = (Invoke-GitLines -Root $Root -GitArguments @('diff', '--raw', $range, '--')) -join "`n"
    if ($rawDiff.Contains('160000', [StringComparison]::Ordinal)) {
...
if ([int]$configuration.schemaVersion -ne 1) {
    throw "Unsupported input schemaVersion '$($configuration.schemaVersion)'."
}

$root = @(Invoke-GitLines -Root (Split-Path $resolvedInputPath -Parent) -GitArguments @('rev-parse', '--show-toplevel'))[0]
$preexistingPath = Join-Path $root (ConvertTo-NormalizedPath ([string]$configuration.preexistingStatePath))
...
$resolvedTrxPath = if ([string]::IsNullOrWhiteSpace($TestResultPath)) {
    Join-Path $root (ConvertTo-NormalizedPath ([string]$configuration.live.testResultPath))
}
else {
...
$allPassed = $liveResult.status -eq 'pass' -and @($historicalResults | Where-Object { $_.status -notlike 'pass*' }).Count -eq 0
$report = [ordered]@{
    schemaVersion = 1
...
$jsonPath = Join-Path $root (ConvertTo-NormalizedPath ([string]$configuration.output.json))
$markdownPath = Join-Path $root (ConvertTo-NormalizedPath ([string]$configuration.output.markdown))
...
if (-not $allPassed) {
    $messages = @($liveResult.failures) + @($historicalResults | ForEach-Object { $_.failures })
