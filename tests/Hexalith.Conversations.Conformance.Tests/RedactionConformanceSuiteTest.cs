// <copyright file="RedactionConformanceSuiteTest.cs" company="ITANEO">
...
/// <summary>
/// Verifies the redaction replay conformance sub-suite covers all 10 required scenarios, produces
/// machine-readable content-safe CI-suitable results, and feeds the redaction-non-leakage release gate mapping.
/// </summary>
public sealed class RedactionConformanceSuiteTest
{
    private static readonly JsonSerializerOptions WebOptions = new(JsonSerializerDefaults.Web);
    private static readonly DateTimeOffset EvaluatedAt = new(2026, 5, 23, 0, 0, 0, TimeSpan.Zero);
    private static readonly IReadOnlyList<RedactionReplayScenarioData> Scenarios = RedactionConformanceSeedData.Scenarios;
    private const string CorrelationId = "rdx-conformance-corr-001";

    private static ConformanceRunResultV1 Run()
        => new RedactionConformanceSuite().Run(Scenarios, CorrelationId, EvaluatedAt);
    ...
    public void EachScenarioShouldProduceExpectedConformanceOutcome()
    {
        ...
        foreach (RedactionReplayScenarioData scenario in Scenarios)
        {
    ...
    [Fact]
    public void AllChecksShouldCarryFR89RequirementAndRedactionGateMappings()
    {
        ConformanceRunResultV1 run = Run();

        foreach (ConformanceCheckResultV1 check in run.Checks)
        {
            check.RequirementMappings.ShouldContain("FR89");
            check.PreconditionMappings.ShouldNotBeEmpty();
            check.ReleaseGateMappings.ShouldContain("redaction-non-leakage");
        }
    }
    ...
    {
        ConformanceRunResultV1 run = Run();
        run.SuiteId.ShouldBe("redaction-conformance-suite");
    ...
    public void RunResultShouldSerializeToStableCamelCaseJsonAndRoundTrip()
    {
        ...
        json.ShouldContain("\"suiteId\":\"redaction-conformance-suite\"");
    ...
    {
        RedactionConformanceSuite suite = new();
