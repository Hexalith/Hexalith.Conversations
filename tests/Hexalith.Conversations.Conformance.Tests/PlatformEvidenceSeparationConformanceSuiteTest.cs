// <copyright file="PlatformEvidenceSeparationConformanceSuiteTest.cs" company="ITANEO">
...
/// <summary>
/// Verifies the platform evidence separation conformance sub-suite covers all 10 required scenarios, produces
/// machine-readable content-safe CI-suitable results, and feeds the platform-evidence release gate mapping.
/// </summary>
public sealed class PlatformEvidenceSeparationConformanceSuiteTest
{
    ...
    private static ConformanceRunResultV1 Run()
        => new PlatformEvidenceSeparationConformanceSuite().Run(Scenarios, CorrelationId, EvaluatedAt);
    ...
    public void EachScenarioShouldProduceExpectedConformanceOutcome()
    {
        ...
        foreach (PlatformEvidenceSeparationScenarioData scenario in Scenarios)
        {
    ...
    [Fact]
    public void AllChecksShouldCarryFR94RequirementAndPlatformEvidenceGateMappings()
    {
        ConformanceRunResultV1 run = Run();

        foreach (ConformanceCheckResultV1 check in run.Checks)
        {
            check.RequirementMappings.ShouldContain("FR94");
            check.PreconditionMappings.ShouldNotBeEmpty();
            check.ReleaseGateMappings.ShouldContain("platform-evidence");
        }
    }
    ...
    {
        ConformanceRunResultV1 run = Run();
        run.SuiteId.ShouldBe("platform-evidence-conformance-suite");
    ...
    public void RunResultShouldSerializeToStableCamelCaseJsonAndRoundTrip()
    {
        ...
        json.ShouldContain("\"suiteId\":\"platform-evidence-conformance-suite\"");
    ...
    {
        PlatformEvidenceSeparationConformanceSuite suite = new();
