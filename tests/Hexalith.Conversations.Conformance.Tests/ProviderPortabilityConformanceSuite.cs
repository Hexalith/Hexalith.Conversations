// <copyright file="ProviderPortabilityConformanceSuite.cs" company="ITANEO">
...
/// <summary>
/// Runs the provider portability conformance sub-suite against a deterministic synthetic scenario list and
/// produces a machine-readable, content-safe <see cref="ConformanceRunResultV1"/> suitable for CI consumption.
/// </summary>
/// <remarks>
/// The suite targets the <c>provider-portability</c> release gate mapping exclusively. It is read-only: no
...
/// as the check ID and map to <c>FR90</c>.
/// </remarks>
public sealed class ProviderPortabilityConformanceSuite
{
    ...
    public ConformanceRunResultV1 Run(
        IReadOnlyList<ProviderPortabilityScenarioData> scenarios,
        ...
        DateTimeOffset evaluatedAt)
    {
        ...
        return new ConformanceRunResultV1(
            ...
            anyFailure
                ? "One or more provider portability scenarios failed conformance."
                : "All provider portability scenarios conform to expected behaviour.",
            "portability-conformance-suite",
            "local-ci-runner",
            correlationId,
            evaluatedAt,
            results);
