// <copyright file="TenantIsolationConformanceSuiteTest.cs" company="ITANEO">
...
/// <summary>
/// Verifies the tenant isolation conformance sub-suite covers all 12 required scenarios, produces
/// machine-readable content-safe CI-suitable results, and feeds the tenant-isolation release gate.
/// </summary>
public sealed class TenantIsolationConformanceSuiteTest
{
    private static readonly JsonSerializerOptions WebOptions = new(JsonSerializerDefaults.Web);
    private static readonly DateTimeOffset EvaluatedAt = new(2026, 5, 23, 0, 0, 0, TimeSpan.Zero);
    private const string CorrelationId = "ti-conformance-corr-001";

    private static ConformanceRunResultV1 Run()
        => new TenantIsolationConformanceSuite().Run(TenantIsolationConformanceSeedData.Scenarios, CorrelationId, EvaluatedAt);
    ...
    [Fact]
    public void AllChecksShouldUseTenantBindingCheckId()
    {
        ConformanceRunResultV1 run = Run();
        run.Checks.ShouldAllBe(check => check.Check.Equals(ConformanceCheck.TenantBinding));
    }
    ...
    public void EachScenarioShouldProduceExpectedConformanceOutcome()
    {
        ...
        foreach (TenantIsolationScenarioData scenario in TenantIsolationConformanceSeedData.Scenarios)
        {
    ...
    [Fact]
    public void AllChecksShouldCarryFR87RequirementAndTenantIsolationGateMappings()
    {
        ConformanceRunResultV1 run = Run();

        foreach (ConformanceCheckResultV1 check in run.Checks)
        {
            check.RequirementMappings.ShouldContain("FR87");
            check.PreconditionMappings.ShouldNotBeEmpty();
            check.ReleaseGateMappings.ShouldContain("tenant-isolation");
        }
    }
    ...
    public void BlockedScenariosShouldHaveNonNullTypedError()
    {
        ...
        blockedChecks.Count().ShouldBe(7);
    ...
    public void UnknownScenariosShouldCarryAggregateNotFoundTypedError()
    {
        ...
        unknownChecks.Count().ShouldBe(3);
    ...
    {
        ConformanceRunResultV1 run = Run();
        run.SuiteId.ShouldBe("isolation-conformance-suite");
    ...
    public void RunResultShouldSerializeToStableCamelCaseJsonAndRoundTrip()
    {
        ...
        json.ShouldContain("\"suiteId\":\"isolation-conformance-suite\"");
    ...
    {
        TenantIsolationConformanceSuite suite = new();
    ...
    public void NullCorrelationIdShouldThrow()
    {
        TenantIsolationConformanceSuite suite = new();
        Should.Throw<ArgumentException>(() =>
            suite.Run(TenantIsolationConformanceSeedData.Scenarios, null!, EvaluatedAt));
    }
}
