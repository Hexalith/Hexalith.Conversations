// <copyright file="ContractValidationConformanceSuiteTest.cs" company="ITANEO">
...
/// <summary>
/// Verifies the contract validation conformance sub-suite covers all 10 required scenarios, produces
/// machine-readable content-safe CI-suitable results, and feeds the contract-compatibility release gate mapping.
/// </summary>
public sealed class ContractValidationConformanceSuiteTest
{
    private static readonly JsonSerializerOptions WebOptions = new(JsonSerializerDefaults.Web);
    private static readonly DateTimeOffset EvaluatedAt = new(2026, 5, 23, 0, 0, 0, TimeSpan.Zero);
    private static readonly IReadOnlyList<ContractValidationScenarioData> Scenarios = ContractValidationConformanceSeedData.Scenarios;
    private const string CorrelationId = "cv-conformance-corr-001";

    private static ConformanceRunResultV1 Run()
        => new ContractValidationConformanceSuite().Run(Scenarios, CorrelationId, EvaluatedAt);
    ...
    [Fact]
    public void AllChecksShouldUseCompatibilityDiscoveryCheckId()
    {
        ConformanceRunResultV1 run = Run();
        run.Checks.ShouldAllBe(check => check.Check.Equals(ConformanceCheck.CompatibilityDiscovery));
    }
    ...
    public void EachScenarioShouldProduceExpectedConformanceOutcome()
    {
        ...
        foreach (ContractValidationScenarioData scenario in Scenarios)
        {
    ...
    [Fact]
    public void AllChecksShouldCarryFR92RequirementAndContractCompatibilityGateMappings()
    {
        ConformanceRunResultV1 run = Run();

        foreach (ConformanceCheckResultV1 check in run.Checks)
        {
            check.RequirementMappings.ShouldContain("FR92");
            check.PreconditionMappings.ShouldNotBeEmpty();
            check.ReleaseGateMappings.ShouldContain("contract-compatibility");
        }
    }
    ...
    public void BlockedScenariosShouldHaveNonNullTypedError()
    {
        ...
        blockedChecks.Count().ShouldBe(1);
    ...
    {
        ConformanceRunResultV1 run = Run();
        run.SuiteId.ShouldBe("contract-validation-conformance-suite");
    ...
    public void RunResultShouldSerializeToStableCamelCaseJsonAndRoundTrip()
    {
        ...
        json.ShouldContain("\"suiteId\":\"contract-validation-conformance-suite\"");
    ...
    {
        ContractValidationConformanceSuite suite = new();
