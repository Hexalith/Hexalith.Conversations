...
/// the enforcement language is asserted inside that span, and three mutations — heading removed, body
/// gutted, clause displaced outside the span — are asserted to fail. A guard that cannot be demonstrated
...
public sealed class StoryFinalRecordGenerationValidationTest
{
    ...
    /// <summary>
    /// Per gated surface: the gate marker, the marker that bounds its span, and the enforcement clauses
    /// that must survive inside that span. The follower marker is load-bearing — without it the span runs
    /// to the end of the file and a displaced clause would count as "inside the gate".
    /// </summary>
    private static readonly (string Path, string GateMarker, string FollowerMarker, string[] Clauses)[] Contracts =
    [
        (
    ...
    public void GeneratorMustExistAndExposeTheContractedInvocationSurface()
    {
        ...
        foreach (string flag in new[] { "--story", "--baseline", "--candidate", "--test-results", "--submodule", "--require-remote", "--historical" })
        {
            generator.ShouldContain($"\"{flag}\"", Case.Sensitive, $"The generator must accept {flag}.");
        }

        // Every blocking condition the four surfaces rely on has to be reachable in the generator itself.
        ...
        {
            generator.ShouldContain(code, Case.Sensitive, $"The frozen blocker {code} must exist in the generator.");
        }
    }
    ...
    {
        foreach ((string path, string gateMarker, string followerMarker, string[] clauses) in Contracts)
        {
            foreach (string tree in new[] { ClaudeTree, AgentTree })
            {
                string content = ReadRepositoryFile($"{tree}/{path}");
                GateContractViolations(content, gateMarker, followerMarker, clauses)
    ...
    public void RemovingTheGateHeadingMustBeDetected()
    {
        foreach ((string path, string gateMarker, string followerMarker, string[] clauses) in Contracts)
        {
            ...
            GateContractViolations(mutated, gateMarker, followerMarker, clauses)
    ...
    public void GuttingTheGateBodyWhileKeepingTheHeadingMustBeDetected()
    {
        foreach ((string path, string gateMarker, string followerMarker, string[] clauses) in Contracts)
        {
            ...
            foreach (string clause in clauses)
            {
                ...
                GateContractViolations(gutted, gateMarker, followerMarker, clauses)
    ...
    {
        string[] skillPaths = [.. StoryFileList().Where(static path => path.StartsWith($"{ClaudeTree}/", StringComparison.Ordinal) || path.StartsWith($"{AgentTree}/", StringComparison.Ordinal))];

        skillPaths.ShouldNotBeEmpty("The story must list the skill files it changed.");

        foreach (string path in skillPaths)
        {
            int index = path.IndexOf("/skills/", StringComparison.Ordinal);
            string relative = path[(index + "/skills/".Length)..];
        ...
        foreach ((string path, _, _, _) in Contracts)
        {
            skillPaths.ShouldContain($"{ClaudeTree}/{path}", $"The story must list {ClaudeTree}/{path}.");
            skillPaths.ShouldContain($"{AgentTree}/{path}", $"The story must list {AgentTree}/{path}.");
        }
    }
    ...
    private static List<string> GateContractViolations(string content, string gateMarker, string followerMarker, string[] clauses)
    {
        ...
        violations.AddRange(clauses
            .Where(clause => !span.Contains(clause, StringComparison.Ordinal))
            .Select(static clause => $"missing enforcement clause: {clause}"));
