// <copyright file="EventSchemaEvolutionConformanceSuiteTest.cs" company="ITANEO">
...
/// <summary>
/// Verifies the event schema evolution conformance sub-suite covers all 10 required scenarios, produces
/// machine-readable content-safe CI-suitable results, and feeds the unsupported-schema-rejection release gate mapping.
/// </summary>
public sealed class EventSchemaEvolutionConformanceSuiteTest
{
    private static readonly JsonSerializerOptions WebOptions = new(JsonSerializerDefaults.Web);
    private static readonly DateTimeOffset EvaluatedAt = new(2026, 5, 23, 0, 0, 0, TimeSpan.Zero);
    private static readonly IReadOnlyList<EventSchemaEvolutionScenarioData> Scenarios = EventSchemaEvolutionConformanceSeedData.Scenarios;
    private const string CorrelationId = "sch-conformance-corr-001";

    private static ConformanceRunResultV1 Run()
        => new EventSchemaEvolutionConformanceSuite().Run(Scenarios, CorrelationId, EvaluatedAt);
    ...
    public void EachScenarioShouldProduceExpectedConformanceOutcome()
    {
        ...
        foreach (EventSchemaEvolutionScenarioData scenario in Scenarios)
        {
    ...
    [Fact]
    public void AllChecksShouldCarryFR91RequirementAndSchemaEvolutionGateMappings()
    {
        ConformanceRunResultV1 run = Run();

        foreach (ConformanceCheckResultV1 check in run.Checks)
        {
            check.RequirementMappings.ShouldContain("FR91");
            check.PreconditionMappings.ShouldNotBeEmpty();
            check.ReleaseGateMappings.ShouldContain("unsupported-schema-rejection");
        }
    }
    ...
    {
        ConformanceRunResultV1 run = Run();
        run.SuiteId.ShouldBe("schema-evolution-conformance-suite");
    ...
    public void RunResultShouldSerializeToStableCamelCaseJsonAndRoundTrip()
    {
        ...
        json.ShouldContain("\"suiteId\":\"schema-evolution-conformance-suite\"");
    ...
    {
        EventSchemaEvolutionConformanceSuite suite = new();
