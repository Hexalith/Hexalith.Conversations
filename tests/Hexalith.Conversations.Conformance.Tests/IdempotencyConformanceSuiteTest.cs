// <copyright file="IdempotencyConformanceSuiteTest.cs" company="ITANEO">
...
/// <summary>
/// Verifies the idempotency conformance sub-suite covers all 8 required scenarios, produces
/// machine-readable content-safe CI-suitable results, and feeds the idempotency release gate mapping.
/// </summary>
public sealed class IdempotencyConformanceSuiteTest
{
    private static readonly JsonSerializerOptions WebOptions = new(JsonSerializerDefaults.Web);
    private static readonly DateTimeOffset EvaluatedAt = new(2026, 5, 23, 0, 0, 0, TimeSpan.Zero);
    private static readonly IReadOnlyList<IdempotencyScenarioData> Scenarios = IdempotencyConformanceSeedData.Scenarios;
    private const string CorrelationId = "idp-conformance-corr-001";

    private static ConformanceRunResultV1 Run()
        => new IdempotencyConformanceSuite().Run(Scenarios, CorrelationId, EvaluatedAt);

    [Fact]
    public void RunResultShouldHaveExactly8Checks()
    {
        ConformanceRunResultV1 run = Run();
        run.Checks.Count.ShouldBe(8);
    }

    [Fact]
    public void AllChecksShouldUseIdempotencyCheckId()
    {
        ConformanceRunResultV1 run = Run();
        run.Checks.ShouldAllBe(check => check.Check.Equals(ConformanceCheck.Idempotency));
    }
    ...
    public void EachScenarioShouldProduceExpectedConformanceOutcome()
    {
        ...
        foreach (IdempotencyScenarioData scenario in Scenarios)
        {
    ...
    [Fact]
    public void AllChecksShouldCarryFR88RequirementAndIdempotencyMappings()
    {
        ConformanceRunResultV1 run = Run();

        foreach (ConformanceCheckResultV1 check in run.Checks)
        {
            check.RequirementMappings.ShouldContain("FR88");
            check.PreconditionMappings.ShouldNotBeEmpty();
            check.ReleaseGateMappings.ShouldContain("idempotency");
        }
    }
    ...
    {
        ConformanceRunResultV1 run = Run();
        run.SuiteId.ShouldBe("idempotency-conformance-suite");
    ...
    public void RunResultShouldSerializeToStableCamelCaseJsonAndRoundTrip()
    {
        ...
        json.ShouldContain("\"suiteId\":\"idempotency-conformance-suite\"");
    ...
    {
        IdempotencyConformanceSuite suite = new();
