...
- `tests/Hexalith.Conversations.Tests/Idempotency/ConversationIdempotencyOutcomeTest.cs`
...
#### Round 2 — Adversarial parallel review (2026-05-20)

3 layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor). Headlines:

- Blind Hunter: 0 BLOCKER, 13 MAJOR, 10 MINOR, 2 NIT, 2 ABSTAIN
- Edge Case Hunter: 1 BLOCKER, 8 MAJOR, 11 MINOR
- Acceptance Auditor: AC1 PARTIAL, AC2 PASS, AC3 PARTIAL, AC4 PARTIAL, AC5 PARTIAL, AC6 PASS — 0 BLOCKER, 1 MAJOR, 6 MINOR

After triage and deduplication: 5 decision-needed, 14 patches, 2 deferrals, 18 dismissed.

#### Decisions resolved (2026-05-20)

- D7 → Patch **P42**: `ConversationProjectionAccumulator` constructor accepts the expected `(TenantId expectedTenantId, ConversationId expectedConversationId)` identity. Any event whose Metadata doesn't match is rejected before `_processedEventIds` / `CaptureIdentity` runs. Remove the "first event sets identity" semantic. Update all `ConversationProjectionAccumulatorTest` cases to construct with the expected identity, and add a test asserting that a pre-Created event from a DIFFERENT tenant/conversation does NOT poison the accumulator. (Resolves Edge Case Hunter F14 BLOCKER.)
- D8 → Patch **P43**: On retryable outcome (`IsRetryable=true`), uncertain outcome (`Category=Uncertain`), `outcomeFactory` exception, and `CompleteAsync` exception, the executor must persist the record as `Poisoned` with a non-null `Outcome=Uncertain` rather than release the reservation. This preserves a durable "something happened here" trace and forces the next caller to resolve from EventStore replay (per ADR-0001 §6 "EventStore history wins"). Add a new `InMemoryConversationIdempotencyStore.MarkPoisonedAsync(scope, outcome, now)` method (and the interface contract) that atomically transitions Pending → Poisoned. Update `IdempotentConversationCommandExecutor.ExecuteReservedAsync` to call `MarkPoisonedAsync` in place of `TryReleaseAsync` for these paths. Reservation token semantics (P32) still apply.
- D9 → Patch **P44**: Preserve the original `ReasonCode` from the stored outcome on duplicate replay of a `Rejection`. Extend `ConversationIdempotencyOutcome` to carry `OriginalReasonCode` alongside `RejectionCode`; the AddParticipant handler populates it at first-attempt time. The executor's `Duplicate when StoredOutcome is { Category: Rejection }` branch uses `outcome.OriginalReasonCode` when emitting the replay event. Caller-shape parity per AC1.
- D10 → Patch **P45**: Treat `ConversationMetadataUpdated.Attributes = empty-dict` as "no change" in `ConversationProjectionAccumulator.Apply(ConversationMetadataUpdated)`, symmetric with the null Label/BusinessReference / null Attributes path. Fingerprint canonicalization already aligns (empty-dict produces no parts via `SafeAttributes`). Loses the "clear all attributes" capability in v1 — acceptable per AC scope; if a future story needs explicit deletion, add a typed `AttributeRemoved` event rather than overloading `MetadataUpdated`.
- D11 → Patch **P46**: Apply NFC normalization in `ConversationPayloadFingerprint.FromParts` to safe payload text fields ONLY (Label, BusinessReference value, attribute values, and other free-form text). Keep byte-exact for `TenantId.Value`, `ConversationId.Value`, `MessageId.Value`, `PartyId.Value`, `FileId.Value`, `IdempotencyKey`, `SchemaVersion`, and `CommandType`. Add an ADR-0001 §"Unicode normalization" clause documenting the asymmetric policy and the negative tests proving identity dimensions stay strict.

#### Patches (apply after decisions)

- [x] [Review][Patch] P27 — `InMemoryConversationIdempotencyStore.CompleteAsync` must require `existing.Status == Pending && existing.RecordVersion == CurrentRecordVersion`. Today it overwrites Completed/Poisoned/VersionIncompatible records silently. [src/Hexalith.Conversations/Idempotency/InMemoryConversationIdempotencyStore.cs:84-105]
- [x] [Review][Patch] P28 — Tighten `IConversationIdempotencyStore.CompleteAsync` XML contract to state the Pending-only precondition. Defensive contract documentation. [src/Hexalith.Conversations/Idempotency/IConversationIdempotencyStore.cs:21-50]
- [x] [Review][Patch] P29 — `AddParticipantCommandHandler` must fail closed (e.g., `InvalidOperationException` or typed rejection) when `_idempotencyExecutor` is wired but `command.Metadata.IdempotencyKey` is null/whitespace. P24 made the key mandatory at the envelope validator, but the handler currently bypasses idempotency rather than failing. [src/Hexalith.Conversations.Server/CommandHandlers/AddParticipantCommandHandler.cs:300-333]
- [x] [Review][Patch] P30 — `ConversationAuditHandle.FromServerBoundary` hash material must include `Scope.ScopeValue` and `Scope.IdempotencyKey`. Today two distinct conversations under the same tenant+command+schema sharing the same `serverOperationId` produce the SAME audit handle. [src/Hexalith.Conversations/Idempotency/ConversationAuditHandle.cs:24-37]
- [x] [Review][Patch] P31 — Remove the `AuditHandle ?? CorrelationId` fallback in `ConversationIdempotencyOutcome`. The D1 review decision said the handle must be server-derived; the fallback silently lets caller-supplied `CorrelationId` become the audit token. Make `auditHandle` mandatory at every factory entry point. [src/Hexalith.Conversations/Idempotency/ConversationIdempotencyOutcome.cs:64, 181-260]
- [x] [Review][Patch] P32 — `InMemoryConversationIdempotencyStore.ReleaseAsync` must verify reservation identity (e.g., compare `existing.CreatedAt` with the value observed at Reserve time, or accept a reservation token from `ReserveAsync`). Today a delayed `Release` from an old caller can remove a fresh `Pending` reservation owned by a newer caller after expiry-replace. [src/Hexalith.Conversations/Idempotency/InMemoryConversationIdempotencyStore.cs:122-127]
- [x] [Review][Patch] P33 — P5 expiry-evict-and-replace must NOT silently overwrite Completed records. Only Pending expired records may be evicted; Completed expired records should return `RetryableUncertainty` (or be resolved from EventStore replay). Today the current behavior contradicts ADR-0001 §6 ("Expired records are not silently overwritten when doing so could duplicate a business effect"). [src/Hexalith.Conversations/Idempotency/InMemoryConversationIdempotencyStore.cs:54-60]
- [x] [Review][Patch] P34 — Add Outcome consistency invariants in `ConversationIdempotencyOutcome.ValidateCategoryInvariant`: a `Rejection` outcome's `IsRetryable` must equal `ConversationErrorCode.IsRetryable(rejectionCode)`; and reject `IsRetryable=true` paired with non-retryable codes. Also tighten `ConversationIdempotencyRecord.ValidateOutcome` to reject `Status=Pending` with a non-null `Outcome`. [src/Hexalith.Conversations/Idempotency/ConversationIdempotencyOutcome.cs:181-260; src/Hexalith.Conversations/Idempotency/ConversationIdempotencyRecord.cs:103-115]
- [x] [Review][Patch] P35 — `CoarsePublicReason` switch in `IdempotentConversationCommandExecutor` must include the `idempotency_duplicate_rejection_replay` and `idempotency_duplicate` reason codes used in the executor itself; today they fall through the default arm. [src/Hexalith.Conversations.Server/CommandHandlers/IdempotentConversationCommandExecutor.cs:114-125]
- [x] [Review][Patch] P36 — `IdempotentConversationCommandExecutor` should consume `TimeProvider` (injected) rather than `DateTimeOffset.UtcNow` for the completion timestamp. P15 broke test determinism — tests that supply a deterministic `now` now see a wall-clock-derived `UpdatedAt`. [src/Hexalith.Conversations.Server/CommandHandlers/IdempotentConversationCommandExecutor.cs:132-138, 158-162]
- [x] [Review][Patch] P37 — `ConversationIdempotencyReplayResult.ResultPayload` should be a computed property, not a once-initialized field. Today `record with { Outcome = newOutcome }` produces an instance whose `Outcome` and `ResultPayload` diverge silently. [src/Hexalith.Conversations/Idempotency/ConversationIdempotencyReplayResult.cs:27-43]
- [x] [Review][Patch] P38 — `ConversationProjectionAccumulator.Apply(...)` methods use `e?.Metadata` followed by `TryMarkProcessed(metadata)` where `TryMarkProcessed` throws on null. The `e?.Metadata` syntax is misleading. Either reject null events at the boundary (throw with a clear message) or genuinely tolerate null by returning early. [src/Hexalith.Conversations.Server/Projections/ConversationProjectionAccumulator.cs:48-110]
- [x] [Review][Patch] P39 — `ConversationCommandFingerprint.Required(key, value)` validates lazily inside a `yield`-iterator method. The `ArgumentException` is thrown deep inside `OrderBy` / `StringBuilder.Append`, and the `paramName` is the JSON canonicalization key (e.g., `"message.id"`), not a real method parameter. Validate eagerly with a typed `ArgumentException` whose message clearly identifies the canonical field. [src/Hexalith.Conversations/Idempotency/ConversationCommandFingerprint.cs:175-206]
- [x] [Review][Patch] P40 — Story `File List` references `1-6-add-idempotency-local-evidence.md`, but the actual file is `1-6-idempotency-local-evidence.md` (no `add-` prefix). One-name drift; fix the File List. [_bmad-output/implementation-artifacts/1-6-add-idempotent-command-handling.md:268]
- [x] [Review][Patch] P41 — Add a `ConversationProjectionAccumulatorTest` covering `ConversationClosed` arriving before `ConversationCreated` (lifecycle reorder safety). Current tests only cover child events before Created, not lifecycle events. [tests/Hexalith.Conversations.Server.Tests/Projections/ConversationProjectionAccumulatorTest.cs]
- [x] [Review][Patch] P42 (from D7) — `ConversationProjectionAccumulator` constructor accepts `(TenantId expectedTenantId, ConversationId expectedConversationId)`; any event whose `Metadata` doesn't match is rejected before identity capture. Add a test asserting a pre-`ConversationCreated` event from a DIFFERENT tenant/conversation does NOT poison the accumulator. Update all existing accumulator tests to construct with explicit identity. [src/Hexalith.Conversations.Server/Projections/ConversationProjectionAccumulator.cs:183-212]
- [x] [Review][Patch] P43 (from D8) — Add `IConversationIdempotencyStore.MarkPoisonedAsync(scope, outcome, now)` and an `InMemoryConversationIdempotencyStore` implementation that atomically transitions `Pending → Poisoned` with a non-null `Outcome=Uncertain`. Update `IdempotentConversationCommandExecutor.ExecuteReservedAsync` to call `MarkPoisonedAsync` instead of `TryReleaseAsync` on `IsRetryable=true`, `Category=Uncertain`, `outcomeFactory` exception, and `CompleteAsync` exception. Reservation-identity verification (P32) still applies. [src/Hexalith.Conversations/Idempotency/IConversationIdempotencyStore.cs; src/Hexalith.Conversations/Idempotency/InMemoryConversationIdempotencyStore.cs; src/Hexalith.Conversations.Server/CommandHandlers/IdempotentConversationCommandExecutor.cs:124-172]
- [x] [Review][Patch] P44 (from D9) — Extend `ConversationIdempotencyOutcome` with `OriginalReasonCode` alongside `RejectionCode`. `AddParticipantCommandHandler.ToIdempotencyOutcome` populates it at first-attempt time from `ConversationRejectedDomainEvent.ReasonCode`. The executor's `Duplicate when StoredOutcome is { Category: Rejection }` branch emits the replay rejection with `outcome.OriginalReasonCode`. Add a contract test asserting first-attempt and Nth-attempt `(Code, ReasonCode)` are identical for the same scoped key. [src/Hexalith.Conversations/Idempotency/ConversationIdempotencyOutcome.cs; src/Hexalith.Conversations.Server/CommandHandlers/AddParticipantCommandHandler.cs; src/Hexalith.Conversations.Server/CommandHandlers/IdempotentConversationCommandExecutor.cs:78-86]
- [x] [Review][Patch] P45 (from D10) — `ConversationProjectionAccumulator.Apply(ConversationMetadataUpdated)` treats `Attributes=empty-dict` as "no change", symmetric with the null path. Add a test asserting two `MetadataUpdated` events with distinct `Attributes` payloads arriving in either order produce the same terminal state. [src/Hexalith.Conversations.Server/Projections/ConversationProjectionAccumulator.cs:136-148]
- [x] [Review][Patch] P46 (from D11) — Apply NFC normalization in `ConversationPayloadFingerprint.FromParts` to safe payload text fields only (Label, BusinessReference value, attribute values). Keep byte-exact for identity dimensions (`TenantId.Value`, `ConversationId.Value`, durable IDs, `IdempotencyKey`, `SchemaVersion`, `CommandType`). Add an ADR-0001 §"Unicode normalization" clause and add negative tests proving that case/visual-similar/NFKC-vs-NFC variations on TenantId still produce distinct fingerprints. [src/Hexalith.Conversations/Idempotency/ConversationPayloadFingerprint.cs:38-60; docs/adrs/0001-idempotency-contract.md]
