...
_From `/bmad-code-review` of commit `dbb2360` on 2026-05-19. Sources: Blind Hunter (adversarial), Edge Case Hunter (branch/boundary), Acceptance Auditor (AC + Dev Notes)._

#### Decisions resolved (2026-05-19) — converted to patches below

- **D1** Public ReasonCode disclosure → **Collapse public, keep internal**. Public `ConversationRejectedDomainEvent.ReasonCode` mirrors the Story 1.2 ErrorCode token (single non-disclosing value per error code). Internal `DenialReason` enum remains for telemetry / audit handle only. Smallest contract change; preserves Story 1.2 vocabulary discipline + AC2 indistinguishability.
- **D2** Bad-member scope → **Hybrid: narrow role, wide poison**. `UnmappedRole` denies only the affected caller (closed-world during Tenants SDK rollout should not DoS valid members). `TenantProjectionPoisoned` (whitespace/null keys, null Members) keeps tenant-wide deny — deep corruption is the signal.
- **D3** `callerPrincipalId` canonicalization → **Require canonical at boundary**. Auth middleware owns canonicalization; service rejects whitespace/control-char/empty as `MissingCaller` (defense-in-depth) but does not silently normalize. Document the contract on `IConversationTenantAccessService`.
- **D4** Shape-vs-tenant ordering → **Hybrid: schema first, semantic after**. Cheap schema-version check stays before tenant access (the version vocabulary is already public in the contract package). Semantic shape (party id format, type, timestamp) moves behind the tenant gate so the validation vocabulary is not fingerprintable to cross-tenant probes.
- **D5** Logger detail → **Trace-level full exception**. Default-level log keeps the current structured `FailureType={ExceptionTypeName}` tag. Add an `ILogger.LogTrace` emission of the full `ex.ToString()` (incl. inner exceptions, stack) for incident response. Production deployments configure trace OFF for the `Hexalith.Conversations.Server.TenantAccess` category by default.

#### Patches (unambiguous fixes — including 5 resolved-decision items)

- [x] [Review][Patch][D1] Collapse public `ReasonCode` on rejection events — change `ToReasonCode()` to return a single token per `ConversationErrorCode` (e.g., always `"tenant_isolation_violation"` for the catch-all error code). Keep `ConversationTenantAccessDenialReason` (enum) on the in-memory decision for telemetry and audit-handle use only; do not let it cross the durable event boundary. Update the existing test that asserts `rejection.ReasonCode.ShouldBe("tenant_member_missing")` to assert the collapsed token; add a new assertion that internal reason is available via decision/log path. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessDecision.cs:325-360`; `tests/Hexalith.Conversations.Server.Tests/TenantAccess/AddParticipantCommandHandlerTenantAccessTest.cs`]
- [x] [Review][Patch][D2] Narrow `UnmappedRole` to the caller's own record — replace `state.Members.Values.Any(role => !Enum.IsDefined(...))` with a check on the caller's specific role only (after `TryGetValue`). Keep the tenant-wide `Keys.Any(poison)` check and the `Members == null` check (both stay as `TenantProjectionPoisoned`). Add a test: a tenant with one unmapped-role member and one valid `TenantOwner` member — the owner is allowed, the unmapped member is denied. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:880-892`]
- [x] [Review][Patch][D3] Reject malformed `callerPrincipalId` instead of normalizing — service stays case/whitespace-sensitive. Strengthen the entry check to reject `callerPrincipalId.Trim() != callerPrincipalId` and any control character with `MissingCaller`. Document on `IConversationTenantAccessService` that the caller MUST hand a canonical principal id (auth middleware responsibility). Add tests for `" user"`, `"user\t"`, `"user\0"`. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:639-646`; `IConversationTenantAccessService.cs`]
- [x] [Review][Patch][D4] Reorder shape validation — keep schema-version check before the tenant guard (already-public vocabulary), move semantic shape validation (party id, type, timestamp, event id) into `HandleAfterTenantAccessAsync` (post-allow). Re-run shape unit tests to confirm the split. [`src/Hexalith.Conversations.Server/CommandHandlers/AddParticipantCommandHandler.cs:60-80`; `AddParticipantBoundary`]
- [x] [Review][Patch][D5] Add `LogTrace(ex, ...)` for projection-store failures — default `LogError` keeps current `FailureType={TypeName}` tag, add `LogTrace(ex, "Tenant access projection lookup failure detail. Requirement={Requirement}", requirement)` for incident-response detail. Confirm appsettings template includes `"Hexalith.Conversations.Server.TenantAccess": "Information"` for production (trace OFF by default). [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:673-684`]
- [x] [Review][Patch] `loadStateAsync` infrastructure exceptions bypass fail-closed rejection — directory failure produces `ParticipantValidationUnavailableRejection`, but state-load failure propagates raw to the caller. Wrap state-load in the same rejection pattern. [`src/Hexalith.Conversations.Server/CommandHandlers/AddParticipantCommandHandler.cs:80`]
- [x] [Review][Patch] Aggregate tenant id never cross-checked against trusted/granted tenant — `RunAsync` is not passed an `aggregateTenantId`, and `state.TenantId` is not compared to the granted tenant after load. AC1/AC3 require aggregate-tenant cross-check. Add `aggregateTenantId: state?.TenantId` plumbing OR re-validate after load with a typed fail-closed denial. [`src/Hexalith.Conversations.Server/CommandHandlers/AddParticipantCommandHandler.cs:89-105`]
- [x] [Review][Patch] `trustedTenantId ?? command.Metadata.TenantId` silently elevates command body to trusted source — when `trustedTenantId` is null, fallback uses caller-controlled command tenant; mismatch check then vacuously passes. Additionally, `decision.ToRejection(...)` records `command.Metadata.CorrelationId`/`CausationId` (attacker-controlled) into the durable rejection event of the targeted tenant. Reject null trusted tenant explicitly; strip/sanitize correlation+causation ids on the denial path. [`src/Hexalith.Conversations.Server/CommandHandlers/AddParticipantCommandHandler.cs:89-105`]
- [x] [Review][Patch] `ToSafeErrorResult` hard-codes `IsRetryable: false`, dropping the decision's retryable signal — spec explicitly permits safe retryability when it does not distinguish protected records. Propagate `decision.IsRetryable`; update the corresponding test assertion. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessDecision.cs:297-317`]
- [x] [Review][Patch] Tenant canonicalization is incomplete — `TryValidateTenantValue` filters only colon + whitespace; accepts `/`, `\`, `|`, `#`, control chars, NUL, RTL marks, NFC/NFKC ambiguous forms. The `MalformedTenant` enum value documents "blank, prefixed, lossy, or otherwise not canonical" but only colon-prefix is enforced. Define an explicit allowed charset and add unicode normalization check. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:731-748`]
- [x] [Review][Patch] `state.Members` dictionary comparer is not enforced — `TryGetValue` relies on the dictionary's intrinsic comparer. A non-default `ITenantProjectionStore` impl could use `OrdinalIgnoreCase` and silently broaden access matching. Assert `Members.Comparer is StringComparer.Ordinal` or perform lookup via explicit Ordinal comparer. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:885`]
- [x] [Review][Patch] `IConversationTenantProjectionSignal.GetProjectionHealthAsync` returning a null record causes NRE that escapes fail-closed — null-check the awaited result and treat null as `TenantAccessUnavailable` (retryable denial). [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:762-786`]
- [x] [Review][Patch] Optional `IConversationTenantProjectionSignal` interface is fail-open — if the registered store does not implement the signal interface, ALL poison/stale/gap/rollback checks are silently skipped. Add a registration-time assertion (or a test) that the registered `ITenantProjectionStore` implements `IConversationTenantProjectionSignal`. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:757-760`; `ConversationTenantAccessServiceCollectionExtensions.cs`]
- [x] [Review][Patch] `ConversationTenantAccessRequirement` is not validated against the enum — unknown requirement values silently fall through to `_ => false` in `HasPermission` (silent feature breakage); `Allowed` factory accepts unknown requirements and authorizes the operation. Validate via `Enum.IsDefined` at `CheckAccessAsync` entry AND in the `Allowed`/`Denied` factories. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:895-904`; `ConversationTenantAccessDecision.cs:226-240`]
- [x] [Review][Patch] Closed-world test coverage incomplete — no test simulates a future Tenants SDK enum value that passes `Enum.IsDefined` but isn't `Active`/`Disabled` (or a future-named role). Current matrix only tests `(TenantStatus)999` / `(TenantRole)999` which fail `Enum.IsDefined`. Add fixtures for the "named but unmapped" case. [`tests/Hexalith.Conversations.Server.Tests/TenantAccess/ConversationTenantAccessServiceTest.cs`]
- [x] [Review][Patch] Boundary tests use reflection only, not `.csproj` XML inspection — Story 1.1 review lesson explicitly warned vacuous reflection checks. The diff adds Tenants references but does NOT add `.csproj` XML inspection proving Tenants absent from `Contracts` and `Hexalith.Conversations`. Add explicit XML-based forbidden-reference assertions. [`tests/Hexalith.Conversations.IntegrationTests/ScaffoldSmokeTest.cs`; `tests/Hexalith.Conversations.Server.Tests/ServerBoundaryTest.cs`]
- [x] [Review][Patch] Negative DI fail-closed test missing — registration test only proves happy path. Add: `BuildServiceProvider` WITHOUT `AddConversationTenantAccess`, then attempt to resolve the command handler / its access service dep, assert it fails fast (no fallback allow). [`tests/Hexalith.Conversations.Server.Tests/TenantAccess/ConversationTenantAccessRegistrationTest.cs`]
- [x] [Review][Patch] Denial matrix row 10 ("ambiguous, duplicate, malformed, or poisoned") only partially tested — duplicate-membership and contradictory-projection-value cases (e.g., disabled tenant with active members) are absent. Add fixtures. [`tests/Hexalith.Conversations.Server.Tests/TenantAccess/ConversationTenantAccessServiceTest.cs`]
- [x] [Review][Patch] Guard does not re-check cancellation between access decision and protected operation — late cancellation after Allowed but before `protectedOperation` starts is not honored at the guard level. Add `cancellationToken.ThrowIfCancellationRequested()` between the two calls. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessGuard.cs:526-541`]
- [x] [Review][Patch] `Allowed` factory does not re-validate canonical TenantId — only null-checks. Any caller constructing `Allowed` with a non-canonical `TenantId.Value` bypasses canonicalization. Either re-validate canonical form in `Allowed`, or restrict the factory to internal-only callers from the service. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessDecision.cs:226-240`]
- [x] [Review][Patch] Handler-level cancellation-propagation test missing — service has it; handler does not. Add a test calling `handler.HandleAsync(..., cancellationToken: pre-cancelled cts.Token)` asserting `OperationCanceledException` propagates. [`tests/Hexalith.Conversations.Server.Tests/TenantAccess/AddParticipantCommandHandlerTenantAccessTest.cs`]
- [x] [Review][Patch] Tenant-access bypass test asserts only `directory.CallCount.ShouldBe(0)`, not that the access service was invoked — make `IConversationTenantAccessService` a spy with explicit `Invocations`/parameter counters; assert positive invocation with the trusted-tenant/caller arguments. [`tests/Hexalith.Conversations.Server.Tests/TenantAccess/AddParticipantCommandHandlerTenantAccessTest.cs:1289-1307`]
- [x] [Review][Patch] `state.Members` enumeration is non-atomic — three separate iterations (keys-poison, values-role, TryGetValue) may observe different snapshots if the dictionary mutates concurrently. Snapshot once (e.g., `var members = state.Members.ToImmutableDictionary(StringComparer.Ordinal)`) and reuse. Relates to the comparer-enforcement patch above. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:855-892`]

#### Deferred

- [x] [Review][Defer] Cross-tenant existence/membership probing via response timing — signal short-circuit vs full `GetAsync` + O(N) member walk produces measurable latency variance. Architectural concern; constant-time defense adds significant complexity. [`src/Hexalith.Conversations.Server/TenantAccess/ConversationTenantAccessService.cs:648-692, 860-892`] — deferred to hardening/observability ADR or Story 6.x.
- [x] [Review][Defer] `idempotencyTenantId` parameter is plumbed but never derived from `command.Metadata.IdempotencyKey` — cross-tenant idempotency replay invisible to the access service. The plumbing exists; wiring belongs to Story 1.6 (`add-idempotent-command-handling`). [`src/Hexalith.Conversations.Server/CommandHandlers/AddParticipantCommandHandler.cs:55, 90`] — deferred to Story 1.6.

#### Dismissed as noise

- Error-code collapse to `TenantIsolationViolation` for distinct denial reasons — this IS spec-correct (Story 1.2 vocabulary discipline). The real public-disclosure issue is in `ReasonCode`, covered by the Decision item above.
- Tests construct `new TenantId(" tenant-a")` directly — defense-in-depth check is intentionally for paths that bypass `TenantId` construction; service is the final boundary.
...
- 2026-05-19: `/bmad-code-review` applied 5 resolved-decision patches (D1-D5) + 17 unambiguous patches (F2/F3/F4/F5/F8/F10/F11/F12/F14/F17/F19/F20/F21/F22/F23/F25/F26). Build clean, 235 tests pass (93 in Server.Tests, up from 51). Story moved to done. F13 (timing-based existence probing) and F16 (idempotency-key tenant derivation) deferred per `deferred-work.md`.
