...
**Current status: PENDING.** FR-20 and SM-C1 cannot pass until one approved, hash-bound manifest retains every v1 denominator test and maps the fixed seven-category set—tenant isolation, idempotency, contract validation, redaction replay, provider portability, projection freshness, and governance audit-pairing—to exact, fully qualified test IDs. The same artifact must bind requirement IDs, the source commit and build identity, file hashes, and approval evidence; it must also give every required preservation obligation a valid traceability closure with zero orphans. Repo-root `docs/release-evidence/preservation-traceability-manifest-v2.json` remains the stale `2.0.0-draft` predecessor. Successor `docs/release-evidence/preservation-traceability-manifest-v3.json`, version `3.0.0-rc.1`, is prepared separately for Owner review with approval fields intentionally empty; neither artifact constitutes approval. The successor distinguishes active initiative requirements from legacy requirements whose release activation remains pending and un-inferred. Closing a category gap may add tests to the denominator; it may never delete, replace, merge, reclassify, waive, substitute, aggregate away, or otherwise shrink an original v1 test, a later approved addition, or another required preservation obligation.
...
| §14 activation model | `preservation-traceability-manifest-v2.json` remains a stale draft. Successor `3.0.0-rc.1` enumerates the current preservation inventory for Owner review but remains pending approval. No §14 obligation may be treated as active release authority without approved, hash-current per-requirement state/activation evidence. |
