#!/usr/bin/env python3
"""Manifest and sealed NuGet archive contract for Conversations releases."""
...
def load_manifest(path: pathlib.Path = MANIFEST) -> list[ManifestPackage]:
    ...
    for index, item in enumerate(packages, start=1):
        ...
        if not PACKAGE_ID_PATTERN.fullmatch(package_id) or not package_id.startswith("Hexalith.Conversations"):
            raise ValueError(f"package id is outside the Conversations namespace: {package_id}")
...
def read_metadata(path: pathlib.Path, source_revision: str | None = None) -> PackageMetadata:
    ...
    with zipfile.ZipFile(path, "r") as archive:
        ...
        if text("authors") != "Hexalith Contributors" or text("projectUrl") != "https://github.com/Hexalith/Hexalith.Conversations":
        ...
        if repository is None or repository.attrib.get("url") != "https://github.com/Hexalith/Hexalith.Conversations":
...
def validate_internal_dependencies(
    ...
    internal_dependencies = [
        ...
        if dependency_id.casefold().startswith("hexalith.conversations")
    ]
    missing = sorted(
        dependency_id
    ...
    if missing:
        raise ValueError(f"{metadata.package_id} has unpublished Conversations dependencies: {missing}")
    for dependency_id, dependency_version in internal_dependencies:
        ...
        if dependency_id != canonical_id:
            raise ValueError(f"{metadata.package_id} uses noncanonical Conversations dependency ID {dependency_id}")
