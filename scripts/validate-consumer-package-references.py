#!/usr/bin/env python3
"""Build isolated package-only consumers for the Conversations release inventory."""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="hexalith-conversations-package-consumers-") as temporary:
...
<configuration>
  <packageSources>
    <clear />
    <add key="conversations-release" value="PACKAGE_DIRECTORY" />
    <add key="nuget.org" value="https://api.nuget.org/v3/index.json" protocolVersion="3" />
  </packageSources>
  <packageSourceMapping>
    <packageSource key="conversations-release">
      <package pattern="Hexalith.Conversations*" />
    </packageSource>
...
""".replace("PACKAGE_DIRECTORY", str(package_directory)),
        ...
        projects = [
            write_consumer(root, "PrimaryPackagesConsumer", package_ids[:2], version),
            write_consumer(root, "ExtendedPackagesConsumer", package_ids[2:], version),
        ]
